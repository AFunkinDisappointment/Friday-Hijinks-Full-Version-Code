...
class PlayState extends MusicBeatState
{
	...
	function makeHaxeState(usehaxe:String, path:String, filename:String) {
		...
		interp.variables.set("endCareer", function () {
			canPause = false;
	...
	override public function update(elapsed:Float) {
		...
		if ((FlxG.keys.justPressed.ENTER || FlxG.keys.justPressed.ESCAPE) && startedCountdown && canPause)
		{
			...
			openSubState(new RobloxPauseSubState(boyfriend.getScreenPosition().x, boyfriend.getScreenPosition().y, [boyfriend, gf, dad]));
		}
		...
		if (((health <= 0 && !opponentPlayer) || (health >= 2 && opponentPlayer)) && !practiceMode && !duoMode) {
			...
			if (inALoop) {
				...
				else if (SONG.stage == 'haunted-mansion' && SONG.player1 == 'bf') {
