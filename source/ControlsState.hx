    ...
    override function create() {
        ...
        for (i in 0...6) {
			var coolText = switch (i)
			{
				...
				case 4:
					'Vol Up: ${FlxG.save.data.keys.volUp.map(function (key:FlxKey) {return FlxKey.toStringMap.get(key);}).join(",")}';
				case 5:
					'Vol Down: ${FlxG.save.data.keys.volDown.map(function (key:FlxKey) {return FlxKey.toStringMap.get(key);}).join(",")}';
				default:
					'how did we get here';
			}
			...
			if (FlxG.keys.firstJustPressed() == ESCAPE || FlxG.keys.firstJustPressed() == ENTER) {
				if (currentKeys.length != 0) {
					switch (awaitingFor)
					{
						...
						case 4:
							FlxG.save.data.keys.volUp = currentKeys;
						case 5:
							FlxG.save.data.keys.volDown = currentKeys;
					}
