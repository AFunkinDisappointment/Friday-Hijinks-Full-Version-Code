...
class PlayState extends MusicBeatState
{
	...
	public var universalVar:Map<String, Dynamic>;
	...
	override public function create()
	{
		...
		#end
		var recentlyPlayed:Array<String> = CoolUtil.coolTextFile('assets/data/recentlyPlayed.txt');
		if (recentlyPlayed.contains(PlayState.SONG.song)) {
			recentlyPlayed.remove(PlayState.SONG.song);
		}
		recentlyPlayed.insert(0, PlayState.SONG.song);
		if (recentlyPlayed.length >= 4) {
			recentlyPlayed.resize(4);
		}
		var recentlysus = StringTools.replace(recentlyPlayed.toString(), ',', '\n');
		recentlysus = StringTools.replace(recentlysus, '[', '');
		recentlysus = StringTools.replace(recentlysus, ']', '');
		File.saveContent('assets/data/recentlyPlayed.txt', recentlysus);
		Note.getFrames = true;
		Note.getSpecialFrames = true;
		Note.specialNoteJson = null;
		Character.makeStorage = true;
		...
		*/
		...
		if (FileSystem.exists('assets/data/' + songLowercase  + "/preload.txt")) {
			...
			for (i in 0...characters.length) {
				Character.preloadCharacter(characters[i]);
			}
		}
		#end
		...
		songName = new FlxText(songPosBG.x, songPosBG.y, songPosBG.width, SONG.song, 16);
		if (downscroll)
			songName.y -= 3;
		...
		difficTxt.text = DifficultyIcons.changeDifficultyFreeplay(storyDifficulty, 0).text + ' - Disappointing+ ${MainMenuState.version}';
	...
	private function generateSong(dataPath:String):Void
	{
		...
		var useSong; // now can use both 'Voices.ogg' and '(songname)_Voices.ogg' (and music if you're still using that lol)
	...
	override public function update(elapsed:Float) {
		...
		if (hscriptStates.exists("modchart")) {
			...
			scriptableCamera = getHaxeVar("scriptableCamera", "modchart");
			scriptCamPos = getHaxeVar("scriptCamPos", "modchart");
